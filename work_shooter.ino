...
void can1_callback(CAN_FRAME *frame)
{
  ...
  if (homing_stage == 1)
  {
    motors.set_target_rpm(0, 1000); // drive toward sensor; tune rpm if needed
  }
...
void loop()
{
  ...
  if (press_list[5] == 1) { // TRIANGLE rising edge
    ...
    motors.set_target_rpm(0, 0); // optional: release PID so it won't fight manual motion
  }
